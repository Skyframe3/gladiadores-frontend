...
let ROUTES=[
 {id:1,name:"Senderos y Cascada",tag:"POPULAR",exp:"panoramica",img:"img/i15.jpg",gal:5,diff:"MODERADA",dur:"3 hrs",dist:"45 km",desc:"Recorrido 4x4 por parque ecoturístico, mirador y cascada. Perfecta para contemplar y vivir el off road al mismo tiempo.",terrain:["Mirador","Cascada","Bosque"],horarios:["10:00","15:00"],
   units:[
 ...
 {id:2,name:"Ruta Gran Mirador",tag:"EXTREMA",exp:"extrema",img:"img/i5.jpg",gal:8,diff:"EXTREMA",dur:"3 hrs",dist:"80 km",desc:"La ruta más extrema. El mejor mirador de la Sierra Norte. Bosque, rocas y subidas que ponen a prueba todo.",terrain:["Mirador","Rocas","Extremo"],horarios:["09:00","12:00"],
   units:[
 ...
 {id:3,name:"Ruta del Río",tag:"AVENTURA",exp:"panoramica",img:"img/i13.jpg",gal:6,diff:"MODERADA",dur:"1 hr",dist:"50 km",desc:"Primera experiencia perfecta. Muchos cruces de río, bosque abundante y ambiente familiar. Ideal para todos.",terrain:["Río","Bosque","Familiar"],horarios:["10:00","12:00","14:00","16:00"],
   units:[
 ...
 {id:4,name:"La Ruta Clásica",tag:"FAMILIAR",exp:"panoramica",img:"img/i14.jpg",gal:4,diff:"FÁCIL",dur:"1.5 hrs",dist:"38 km",desc:"La ruta que siempre recomendarán. Cueva, fábrica abandonada, río y senderos. Clásica por algo.",terrain:["Clásica","Cueva","Río"],horarios:["10:00","12:00","14:00","16:00"],
   units:[
 ...
 {id:5,name:"Aventura Nocturna",tag:"ESPECIAL",exp:"extrema",img:"img/i5.jpg",gal:5,diff:"MODERADA",dur:"1 hr 50 min",dist:"35 km",desc:"El bosque de noche es otro mundo. Recorrido nocturno con el cielo estrellado como techo y la adrenalina multiplicada.",terrain:["Nocturno","Bosque","Estrellas"],horarios:["18:00"],
   units:[
 ...
 {id:6,name:"Luciérnagas",tag:"TEMPORADA",exp:"panoramica",img:"img/i15.jpg",gal:4,diff:"FÁCIL",dur:"3+ hrs",dist:"25 km",desc:"Una de las experiencias más mágicas de México. Temporada limitada: el bosque se ilumina con miles de luciérnagas.",terrain:["Luciérnagas","Bosque","Temporada"],horarios:["18:00"],
   units:[
 ...
 {id:7,name:"Amanecer en la Montaña",tag:"AVENTURA",exp:"panoramica",img:"img/i14.jpg",gal:6,diff:"AVANZADA",dur:"3+ hrs",dist:"55 km",desc:"Sal antes del alba. Llega al mirador justo cuando el sol rompe el horizonte sobre la Sierra Norte. Imposible de olvidar.",terrain:["Amanecer","Montaña","Paisaje"],horarios:["05:30"],
   units:[
 ...
 {id:8,name:"Experiencia Mezcal",tag:"CULTURAL",exp:"panoramica",img:"img/i13.jpg",gal:5,diff:"MODERADA",dur:"2.5 hrs",dist:"42 km",desc:"Off road y cultura local. Recorre senderos hasta conocer el proceso artesanal del mezcal de la Sierra Norte de Puebla.",terrain:["Mezcal","Cultura","Senderos"],horarios:["11:00","15:00"],
   units:[
 ...
 {id:9,name:"Expedición al Volcán",tag:"ÉPICA",exp:"extrema",img:"img/i5.jpg",gal:7,diff:"AVANZADA",dur:"2 hrs",dist:"60 km",desc:"El volcán más cercano a Chignahuapan. Off road de alto nivel con vistas que no tienen comparación en toda la Sierra Norte.",terrain:["Volcán","Paisaje","Extremo"],horarios:["10:00","15:00"],
   units:[]},
 {id:10,name:"Cascada Iluminada",tag:"NOCTURNA",exp:"extrema",img:"img/i5.jpg",gal:5,diff:"MODERADA",dur:"3 hrs",dist:"45 km",desc:"Parque iluminado, cascada y experiencia 4x4 nocturna. Principalmente viernes, sábado, domingo y fechas especiales.",terrain:["Nocturno","Cascada","Iluminación"],horarios:["18:00"],
   units:[
...
const UNITS_FLEET=[
 ...
 {uid:"commander-2",img:"img/trail-gulfstream.avif",cat:"SSV COMPACTO",name:"Commander Trail",seats:"2 plazas",qty:"1 unidad",tag:"Compacta para parejas"},
...
const MERCH=[{id:1,emo:"",name:"Visor Táctico",price:450,tag:"PROTECCIÓN"},{id:2,emo:"",name:"Jersey Oficial",price:650,tag:"EDICIÓN LTD"},{id:3,emo:"",name:"Guantes de Ruta",price:390,tag:"GRIP PRO"},{id:4,emo:"",name:"Gorra Táctica",price:280,tag:"ACCESORIOS"},{id:5,emo:"",name:"Playera Gladiador",price:320,tag:"ALGODÓN"},{id:6,emo:"",name:"Linterna Táctica",price:395,tag:"EQUIPO"}];
...
function renderRoutes(filter){const f=filter||'todas';lastFilter=f;document.getElementById('routes-grid').innerHTML=ROUTES.filter(r=>f==='todas'||r.exp===f).filter(r=>unitsOf(r).length).map(r=>{const us=unitsOf(r);const tot=us.reduce((s,u)=>s+u.seats,0),bk=us.reduce((s,u)=>s+u.booked.length,0),avl=tot-bk,minp=Math.min(...us.map(desdeDe));const imgStyle=r.img?`background-image:url(${r.img})`:`background:linear-gradient(135deg,var(--card) 0%,rgba(255,122,0,0.12) 50%,rgba(163,214,60,0.08) 100%)`;return `<div class="rc ${r.exp==='extrema'?'extrema':''}" data-a="openRouteFicha" data-p="${r.id}"><div class="rc-img" style="${imgStyle}"><div class="rc-img-ov"></div><div class="rc-tags-top"><span class="rc-tag" style="background:rgb(${colorTag(r)});color:${tinta(colorTag(r))}">${r.tag}</span><span class="rc-tag" style="background:rgb(${colorDif(r)});color:${tinta(colorDif(r))}">${r.diff}</span></div>${r.gal?`<div class="rc-galcount">${r.gal} fotos</div>`:''}<div class="rc-name">${r.name}</div></div><div class="rc-body"><div class="rc-desc">${r.desc}</div><div class="rc-tags">${r.terrain.map(t=>`<span class="rc-tr">${t}</span>`).join('')}</div><div class="rc-stats"><div class="rc-st"><div class="rc-st-lbl">DUR</div><div class="rc-sv">${r.dur}</div><div class="rc-sl">Duración</div></div><div class="rc-st"><div class="rc-st-lbl">DIST</div><div class="rc-sv">${r.dist}</div><div class="rc-sl">Distancia</div></div><div class="rc-st"><div class="rc-st-lbl">ASIENTOS</div><div class="rc-sv" style="color:${avl===0?'#FF5A5A':'var(--ink)'}">${avl}/${tot}</div><div class="rc-sl">Libres</div></div></div><div class="rc-foot"><div class="rc-price"><span class="from">desde</span>$${minp}<small>/unidad</small></div>${avl===0?`<div class="btn-full">LLENO</div>`:`<button class="btn-book" data-stop="1" data-a="openBooking" data-p="${r.id}">RESERVAR →</button>`}</div></div></div>`;}).join('');}
...
function renderMerch(){document.getElementById('merch-grid').innerHTML=MERCH.map(m=>`<div class="mc"><div class="mc-emo">${m.emo}</div><div class="mc-tag">${m.tag}</div><div class="mc-name">${m.name}</div><div class="mc-price">$${m.price}</div><button class="mc-btn" id="mb-${m.id}" data-a="addMerch" data-p="${m.id}">+ CARRITO</button></div>`).join('');}
function addMerch(id){const m=MERCH.find(x=>x.id===id);addToCart({label:`${m.emo} ${m.name}`,price:m.price});const b=document.getElementById('mb-'+id);b.textContent='✓ AÑADIDO';b.classList.add('added');setTimeout(()=>{b.textContent='+ CARRITO';b.classList.remove('added');},1500);}
...
function renderCart(){const its=document.getElementById('cart-its'),ft=document.getElementById('cart-ft');if(!cart.length){its.innerHTML='<div class="cart-emp"><p>Tu carrito está vacío</p></div>';ft.innerHTML='';return;}its.innerHTML=cart.map(i=>`<div class="cart-it"><div><div class="cart-it-nm">${i.label}</div><div class="cart-it-pr">$${i.price}</div></div><button class="cart-rm" data-a="removeFromCart" data-p="${i.cid}">✕</button></div>`).join('');const t=cart.reduce((s,i)=>s+i.price,0);ft.innerHTML=`<div class="cart-tr"><span style="color:var(--muted)">Total</span><span class="cart-tv">$${t}</span></div><button class="btn-fw">PROCEDER AL PAGO</button>`;}
...
function renderRouteFicha(){
  ...
  const viewer=photos.length?`<div class="rf-viewer"><img src="${photos[rfIdx]}" alt="${esc(r.name)} — foto ${rfIdx+1} de ${photos.length}">${photos.length>1?`<button class="rf-nav prev" data-a="rfNav" data-p="-1" aria-label="Foto anterior">‹</button><button class="rf-nav next" data-a="rfNav" data-p="1" aria-label="Foto siguiente">›</button><div class="rf-count">${rfIdx+1}/${photos.length}</div>`:''}</div>${photos.length>1?`<div class="rf-thumbs">${photos.map((p,i)=>`<img class="rf-thumb ${i===rfIdx?'sel':''}" src="${p}" data-a="rfGoto" data-p="${i}" alt="Miniatura ${i+1}">`).join('')}</div>`:''}`:`<div class="rf-viewer" style="display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:13px">Sin fotos todavía</div>`;
  document.getElementById('rf-body').innerHTML=`${viewer}
    <div class="rf-info">
      <div class="rf-tags"><span class="rf-tag" style="background:rgb(${colorTag(r)});color:${tinta(colorTag(r))}">${esc(r.tag)}</span><span class="rf-tag" style="background:rgb(${colorDif(r)});color:${tinta(colorDif(r))}">${esc(r.diff)}</span></div>
    ...
    <div class="rf-footer" style="margin-top:6px">
      <button class="btn-back" style="flex:1" data-a="closeRouteFicha">← VOLVER A RUTAS</button>
      <button class="btn-next" style="flex:1" data-a="rfReservar">RESERVAR ESTA RUTA →</button>
    </div>`;
}
...
function renderStep(){const L=['PASO 1 DE 4 · UNIDAD Y PERSONAS','PASO 2 DE 4 · FECHA Y HORARIO','PASO 3 DE 4 · TUS DATOS',RESERVAS_EN_LINEA?'PASO 4 DE 4 · PAGO':'PASO 4 DE 4 · ENVIAR SOLICITUD'];document.getElementById('mstep').textContent=L[bStep];const bd=document.getElementById('mbody');const bar=`<div class="step-bar">${[0,1,2,3].map(s=>`<div class="step-seg" style="background:${s<=bStep?'var(--fire)':'rgba(255,255,255,.1)'}"></div>`).join('')}</div>`;
if(bStep===0){bd.innerHTML=bar+`<span class="field-lbl">ELIGE TU UNIDAD</span><div style="display:flex;flex-direction:column;gap:12px;margin-bottom:16px">${unitsOf(bRoute).map(u=>{const cap=capacidadDe(u);const sel=bUnit&&bUnit.id===u.id;const unitImg=UNITS_FLEET.find(f=>f.uid===u.id);const imgSrc=unitImg?unitImg.img:'';
const pax=sel?`<div style="width:100%;margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,122,0,0.2)"><div style="color:var(--muted);font-size:12px;margin-bottom:8px">¿Cuántos van?</div><div style="display:flex;gap:8px;flex-wrap:wrap">${tarifasDe(u).map(t=>`<div class="opt ${bPersonas===t.personas?'sel':''}" style="flex:1;min-width:110px;text-align:center" data-stop="1" data-a="selPax" data-p="${t.personas}"><b>${t.personas} ${t.personas===1?'persona':'personas'}</b><div style="font-size:14px;font-weight:800;color:${bPersonas===t.personas?'inherit':'var(--fire)'}">$${t.precio}</div></div>`).join('')}</div></div>`:'';
return `<div class="unit-pick ${sel?'sel':''}" style="flex-wrap:wrap" data-a="selUnit" data-p="${u.id}"><div style="display:flex;align-items:center;gap:14px;flex:1">${imgSrc?`<img src="${imgSrc}" alt="${u.name}" style="width:64px;height:48px;object-fit:contain;border-radius:8px;background:rgba(255,122,0,0.05)">`:''}<div><b>${u.name}</b><small style="display:block">${u.type} · hasta ${cap} ${cap===1?'persona':'personas'}</small></div></div><div class="price">$${desdeDe(u)}<div style="font-size:11px;color:var(--muted);font-weight:400">desde</div></div>${pax}</div>`;}).join('')}</div>${bUnit&&bPersonas?`<div style="background:rgba(255,122,0,0.06);border:1px solid rgba(255,122,0,0.2);border-radius:12px;padding:14px 16px;margin-bottom:16px;text-align:center"><div style="color:var(--muted);font-size:12px">Tu unidad</div><div style="font-family:'Barlow Condensed',sans-serif;font-size:24px;font-weight:900;color:var(--fire);margin:4px 0">${bUnit.name} · ${bPersonas} ${bPersonas===1?'persona':'personas'}</div><div style="color:var(--muted);font-size:13px">Vehículo completo para ustedes · <b style="color:var(--ink)">$${precioDe(bUnit,bPersonas)}</b></div></div>`:''}<div class="btns-row"><button class="btn-next" ${(!bUnit||!bPersonas)?'disabled':''} data-a="goStep" data-p="1">ELEGIR FECHA →</button></div>`;}
...
bd.innerHTML=bar+`<span class="field-lbl">FECHA</span>${hayDias?renderBookCal():`<input type="date" id="input-fecha" class="date-in" min="${hoy}" value="${bFecha||''}" data-bindc="fecha">`}${bFecha&&!fechaValida&&hayDias?'<p style="color:#FF5A5A;font-size:12px;margin-top:4px">Esa fecha no está disponible para esta ruta.</p>':''}<span class="field-lbl" style="margin-top:14px">HORARIO DE SALIDA</span><div class="opt-row">${bRoute.horarios.map(h=>`<div class="opt ${bHorario===h?'sel':''}" data-a="setHorario" data-p="${h}">${h}</div>`).join('')}</div><div class="btns-row" style="margin-top:8px"><button class="btn-back" data-a="goStep" data-p="0">← ATRÁS</button><button class="btn-next" ${(!(hayDias?fechaValida:bFecha)||!bHorario)?'disabled':''} data-a="goStep" data-p="2">TUS DATOS →</button></div>`;}
  ...
  <div style="background:rgba(163,214,60,0.07);border:1px solid rgba(163,214,60,0.28);border-radius:14px;padding:16px 18px;margin-bottom:18px;margin-top:16px">
    <div style="font-weight:700;font-size:14px;color:var(--gladio);margin-bottom:4px">¿Necesitas algo distinto?</div>
    <div style="color:var(--muted);font-size:12.5px;line-height:1.6;margin-bottom:12px">Grupos grandes, cumpleaños, pedida de mano, empresas o cualquier detalle especial.</div>
    <textarea id="input-nota" class="date-in" rows="2" placeholder="Cuéntanos qué necesitas..." style="resize:vertical;min-height:56px;font-family:'Inter',sans-serif" data-bind="nota">${esc(bNota)}</textarea>
  </div>
  <div style="background:var(--card);border:1px solid rgba(255,122,0,0.2);border-radius:12px;padding:14px 16px;margin-bottom:16px;text-align:center"><div style="color:var(--muted);font-size:12px">Subtotal</div><div style="font-family:'Barlow Condensed',sans-serif;font-size:28px;font-weight:900;color:var(--fire)">$${subtotal}</div></div>
  <label style="display:flex;align-items:flex-start;gap:10px;margin-bottom:16px;cursor:pointer" data-prev="1" data-a="tPrivacidad"><div id="chk-privacidad" class="chk-box ${bPrivacidad?'on':''}">${bPrivacidad?'✓':''}</div><span style="font-size:12.5px;color:var(--muted);line-height:1.5">He leído y acepto el <a href="aviso-privacidad.html" target="_blank" data-stop="1" style="color:var(--fire)">Aviso de Privacidad</a> y el tratamiento de mis datos para gestionar esta reserva.</span></label>
  ...
  bd.innerHTML=bar+`<div class="summ-box"><div class="summ-row"><span style="color:var(--fire);font-weight:600">Cliente</span><span>${esc(bNombre)}</span></div><div class="summ-row"><span style="color:var(--muted);font-size:12px">${esc(bEmail)} · ${esc(bWhatsapp)}</span></div><div style="border-top:1px solid rgba(255,255,255,.1);margin:10px 0;padding-top:10px;margin-bottom:10px"><div class="summ-row"><span>${bRoute.name} · ${bFecha} · ${bHorario}</span><span></span></div><div class="summ-row"><span>${bUnit.name} · ${bPersonas} ${bPersonas===1?'persona':'personas'}</span><span>$${total}</span></div></div><div class="summ-tot"><span style="font-weight:600">Total</span><span class="summ-tot-p">$${total}</span></div></div>
  <span class="field-lbl">¿CÓMO QUIERES PAGAR?</span><div class="pay-split"><div class="pay-opt ${bPayMode==='anticipo'?'sel':''}" data-a="setPay" data-p="anticipo"><div class="pay-radio"></div><div class="po-name">Solo anticipo (25%)<div style="font-size:12px;color:var(--muted);font-weight:400">Resto el día de la ruta</div></div><div class="po-amt">$${anticipo}</div></div><div class="pay-opt ${bPayMode==='completo'?'sel':''}" data-a="setPay" data-p="completo"><div class="pay-radio"></div><div class="po-name">Pago completo<div style="font-size:12px;color:var(--muted);font-weight:400">Listo, sin pagar nada más</div></div><div class="po-amt">$${total}</div></div></div>
  ${RESERVAS_EN_LINEA?`<span class="field-lbl">MÉTODO DE PAGO</span><button class="pay-mth sel" style="pointer-events:none"><div><div class="pm-name">Mercado Pago</div><div class="pm-desc">Tarjeta, SPEI, OXXO o transferencia bancaria</div></div></button>
  <div class="btns-row" style="margin-top:6px"><button class="btn-back" data-a="goStep" data-p="2">← ATRÁS</button><button class="btn-next" id="pay-btn" data-a="confirmBooking" data-p="${monto}">PAGAR $${monto} →</button></div>`
  :`<div style="background:rgba(255,122,0,0.08);border:1px solid rgba(255,122,0,0.3);border-radius:14px;padding:18px;margin-bottom:16px">
    <div style="font-weight:700;font-size:15px;color:var(--fire);margin-bottom:6px">Tu lugar se confirma por WhatsApp</div>
    <div style="color:var(--muted);font-size:13px;line-height:1.7">Pronto podrás pagar aquí mismo con Mercado Pago (tarjeta, SPEI u OXXO). Por ahora manda tu solicitud y te contestamos con la disponibilidad y las formas de pago.</div>
  </div>
  <div class="btns-row" style="margin-top:6px"><button class="btn-back" data-a="goStep" data-p="2">← ATRÁS</button><button class="btn-next" data-a="enviarSolicitud">ENVIAR POR WHATSAPP →</button></div>`}`;}}
...
async function enviarSolicitud(){
  ...
  try{
    ...
    document.getElementById('mbody').innerHTML=`<div style="text-align:center;padding:20px;color:#ff6b6b"><div style="font-size:24px;margin-bottom:10px">⚠ ${esc(err.message||'Error de conexión')}</div><button class="btn-fw" style="background:#ff6b6b;margin-top:16px;" data-a="goStep" data-p="3">← REINTENTAR</button></div>`;
  }
}
...
function mostrarTicket(data){
  ...
  <div id="ticket-print" style="text-align:center;padding:8px 0">
    <div style="font-family:'Barlow Condensed',sans-serif;font-size:30px;font-weight:900;color:var(--fire);margin-bottom:4px">RESERVA CONFIRMADA</div>
    <p style="color:var(--muted);font-size:13px;margin-bottom:18px">Se abrió WhatsApp con tu folio. Mándanos el mensaje y confirmamos disponibilidad.</p>
    <div style="background:var(--card);border:1px solid rgba(255,122,0,0.3);border-radius:16px;padding:22px 18px;margin-bottom:16px">
      <img src="img/i1.png" alt="Gladiadores Off Road" width="44" height="44" style="margin-bottom:8px">
      <div style="color:var(--muted);font-size:11px;letter-spacing:1px;text-transform:uppercase">Folio</div>
      <div style="font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:900;color:var(--fire);letter-spacing:1px;margin-bottom:14px">${esc(data.folio)}</div>
      <div style="display:flex;justify-content:center;margin-bottom:14px">${qrSvg}</div>
      <div style="text-align:left;border-top:1px solid rgba(255,255,255,.1);padding-top:14px;font-size:13.5px;color:var(--ink);line-height:2">
        <div><b style="color:var(--muted);font-weight:400">Ruta:</b> ${esc(bRoute.name)}</div>
        <div><b style="color:var(--muted);font-weight:400">Fecha:</b> ${fechaLegible} · ${esc(bHorario)}</div>
        <div><b style="color:var(--muted);font-weight:400">Unidad:</b> ${esc(data.unidad)} · ${bPersonas} ${bPersonas===1?'persona':'personas'}</div>
        <div style="border-top:1px solid rgba(255,255,255,.1);margin-top:6px;padding-top:6px"><b style="color:var(--muted);font-weight:400">${bPayMode==='anticipo'?'Anticipo pagado:':'Pagado:'}</b> <span style="color:var(--fire);font-weight:700">$${data.monto}</span>${bPayMode==='anticipo'?` <span style="color:var(--muted)">· resto en la ruta $${falta}</span>`:''}</div>
      </div>
    </div>
    <p style="color:var(--muted);font-size:12.5px;margin-bottom:18px">Muestra este ticket (o el QR) al llegar a la base. ¿No se abrió WhatsApp? Escríbenos al <b style="color:var(--ink)">797 100 1929</b></p>
    <div style="display:flex;gap:10px">
      <button class="btn-fw" style="background:transparent;border:1px solid rgba(255,255,255,.15);color:var(--ink)" data-a="printTicket">IMPRIMIR / GUARDAR</button>
...
function renderBookCal(){
  ...
  let h=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><button class="opt" data-a="bCalNav" data-p="-1" style="padding:6px 12px">←</button><b style="color:var(--ink);font-size:14px">${meses[bCalM]} ${bCalY}</b><button class="opt" data-a="bCalNav" data-p="1" style="padding:6px 12px">→</button></div>`;
  h+='<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;max-width:360px">';
  ['L','M','X','J','V','S','D'].forEach(d=>{h+=`<div style="text-align:center;font-size:11px;color:var(--muted);font-weight:600;padding:4px 0">${d}</div>`;});
  ...
  for(let d=1;d<=dim;d++){
    ...
    h+=`<div style="${style}" data-a="pickDay" data-p="${iso}">${d}</div>`;
  }
  h+='</div>';
  return h;
}
function tExtra(id){const i=bExtras.indexOf(id);if(i>=0)bExtras.splice(i,1);else bExtras.push(id);renderStep();}
function confirmBooking(amt){const bd=document.getElementById('mbody');const payBtn=document.getElementById('pay-btn');payBtn.disabled=true;payBtn.textContent='Procesando...';const reservaData={nombre:bNombre,email:bEmail,whatsapp:bWhatsapp,ruta:bRoute.name,rutaId:bRoute.id,categoriaId:bUnit.id,unidad:bUnit.name,horario:bHorario,fecha:bFecha,personas:bPersonas,extras:bExtras,monto:amt,modoPago:bPayMode,metodoPago:bPayMethod};fetch('https://gladiadores-backend.vercel.app/api/reservas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(reservaData)}).then(r=>r.json()).then(data=>{if(data.ok&&data.folio){const f=esc(data.folio);addToCart({label:`${bRoute.name} · ${bUnit.name} · ${bPersonas} pers.`,price:precioDe(bUnit,bPersonas)});bExtras.forEach(id=>{const e=EXTRAS.find(x=>x.id===id);addToCart({label:`${e.emo} ${e.name}`,price:e.price});});bd.innerHTML=`<div style="text-align:center;padding:16px 0"><div style="font-family:Barlow Condensed;font-size:36px;font-weight:900;color:var(--fire);margin-bottom:8px">✓ RESERVA CONFIRMADA</div><p style="color:var(--muted);margin-bottom:20px">Folio <b style="color:var(--fire);font-size:24px">${f}</b></p><p style="color:var(--muted);margin-bottom:20px">Pagaste <b style="color:var(--ink)">$${amt}</b> (${bPayMode==='anticipo'?'anticipo 25%':'pago completo'})</p><div style="background:rgba(255,122,0,0.08);border:1px solid rgba(255,122,0,0.2);border-radius:14px;padding:18px;text-align:left;margin-bottom:20px"><div style="display:flex;gap:10px;margin-bottom:12px;align-items:center"><div><b style="color:var(--fire);font-size:14px">✓ Reserva guardada</b><div style="color:var(--muted);font-size:12px">En tu cuenta (${esc(bEmail)})</div></div></div><div style="display:flex;gap:10px;margin-bottom:12px;align-items:center"><div><b style="color:var(--fire);font-size:14px">✓ Unidad apartada</b><div style="color:var(--muted);font-size:12px">El vehículo queda solo para ustedes</div></div></div><div style="display:flex;gap:10px;align-items:center"><div><b style="color:var(--fire);font-size:14px">✓ Admin notificado</b><div style="color:var(--muted);font-size:12px">Te contactaremos en 2 horas</div></div></div></div><button class="btn-fw" data-a="closeBooking">LISTO, CERRAR</button></div>`;}else{payBtn.disabled=false;payBtn.textContent='PAGAR $'+amt+' →';bd.innerHTML=`<div style="text-align:center;padding:20px;color:#ff6b6b"><div style="font-size:24px;margin-bottom:10px">⚠ Error en la reserva</div><p>${esc(data.error)||'No pudimos procesar tu reserva. Intenta de nuevo.'}</p><button class="btn-fw" style="background:#ff6b6b;margin-top:16px;" data-a="goStep" data-p="3">← REINTENTAR</button></div>`;}}).catch(err=>{console.error('Error:',err);payBtn.disabled=false;payBtn.textContent='PAGAR $'+amt+' →';bd.innerHTML=`<div style="text-align:center;padding:20px;color:#ff6b6b"><div style="font-size:24px;margin-bottom:10px">⚠ Error de conexión</div><p>No pudimos conectar con el servidor. Verifica tu conexión e intenta de nuevo.</p><button class="btn-fw" style="background:#ff6b6b;margin-top:16px;" data-a="goStep" data-p="3">← REINTENTAR</button></div>`});}
...
document.getElementById('routes-grid').innerHTML='<p style="grid-column:1/-1;text-align:center;color:var(--muted);padding:40px 0">Cargando rutas…</p>';
